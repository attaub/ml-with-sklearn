...
svm_clf = SVC(kernel="linear", C=1000000.0)
...
def plot_svc_decision_boundary(svm_clf, xmin, xmax):
    ...
    # At the decision boundary, w0*x0 + w1*x1 + b = 0 or w^T x = b
    # => x1 = -w0/w1 * x0 - b/w1
...
svm_poly_reg2 = SVR(
    kernel="poly", degree=2, C=0.01, epsilon=0.1, gamma="scale"
)
...
class MyLinearSVC(BaseEstimator):
    def __init__(
        self, C=1, eta0=1, eta_d=10000, n_epochs=1000, random_state=None
    ):
        self.C = C
        self.eta0 = eta0
...
print(
    "SGDClassifier(alpha={:.5f}):".format(sgd_clf.alpha),
    sgd_clf.intercept_,
    sgd_clf.coef_,
)
...
rnd_search_cv = RandomizedSearchCV(
    svm_clf, param_distributions, n_iter=10, verbose=2, cv=3
)
