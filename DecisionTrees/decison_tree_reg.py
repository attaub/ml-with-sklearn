...
class DTRegressor:
    ...
    def _best_split(self, X, y):
        ...
        for feature_index in range(n_features):
            ...
            for threshold in thresholds:
                ...
                weighted_mse = (len(left_y) / n_samples) * left_mse + (
    ...
    def _prune_node(self, node, X_val, y_val):
        ...
        current_mse = np.mean(
            (self._predict_subtree(node, X_val) - y_val) ** 2
        )
        leaf_mse = np.mean((self._predict_leaf(node, X_val) - y_val) ** 2)

        if leaf_mse <= current_mse:
            node.left = node.right = None
    ...
    def _predict_leaf(self, node, X):
        return np.full(X.shape[0], np.mean(self._predict_subtree(node, X)))
